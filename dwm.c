...
// || C->issticky)
#define ISVISIBLE(C)                                                           \
  ISVISIBLEONTAG(C, C->mon->tagset[C->mon->seltags] || C->issticky)
...
struct Client {
  ...
  int bw, oldbw;
  unsigned int tags;
  int ismax, wasfloating, isfixed, iscentered, isfloating, isurgent, neverfocus, oldstate,
...
static void killunsel(const Arg *arg); // Kill all client except focused
...
void drawtab(Monitor *m) {
  ...
  for (c = m->clients; c; c = c->next) {
    ...
    if (remainder && i == m->ntabs - 1)
      m->tab_widths[i] += remainder;
    w = m->tab_widths[i];
...
void focusstack(const Arg *arg) {
  ...
  if (!selmon->sel ||
      (selmon->sel->isfullscreen && selmon->sel->fakefullscreen != 1))
    return;
  if (arg->i > 0) {
...
// Suckless patch won't work because sendevent has been modified by systray patch
...
void losefullscreen(Client *next) {
  ...
  if (!sel || !next)
    return;
  if (sel->isfullscreen && sel->fakefullscreen != 1 && ISVISIBLE(sel) &&
      sel->mon == next->mon && !next->isfloating)
    setfullscreen(sel, 0);
}
...
void movemouse(const Arg *arg) {
  ...
  if (c->isfullscreen &&
      c->fakefullscreen !=
          1) /* no support moving fullscreen windows by mouse */
    return;
  restack(selmon);
  ocx = c->x;
  ocy = c->y;
  ...
  do {
    ...
    case MapRequest:
      ...
      break;
    case MotionNotify:
      if ((ev.xmotion.time - lasttime) <= (1000 / 500))
        continue;
...
void reorganizetags(const Arg *arg) {
  Client *c;
...
void resizemouse(const Arg *arg) {
  ...
  if (c->isfullscreen &&
      c->fakefullscreen !=
          1) /* no support resizing fullscreen windows by mouse */
    return;
  restack(selmon);
  ocx = c->x;
  ocy = c->y;
...
void setfullscreen(Client *c, int fullscreen) {
  ...
  if ((c->fakefullscreen == 0 && fullscreen &&
       !c->isfullscreen) // normal fullscreen
      || (c->fakefullscreen == 2 &&
          fullscreen)) // fake fullscreen --> actual fullscreen
    savestate = 1;     // go actual fullscreen
  else if ((c->fakefullscreen == 0 && !fullscreen &&
            c->isfullscreen) // normal fullscreen exit
           || (c->fakefullscreen >= 2 &&
               !fullscreen)) // fullscreen exit --> fake fullscreen
    restorestate = 1;        // go back into tiled

  /* If leaving fullscreen and the window was previously fake fullscreen (2),
   * then restore that while staying in fullscreen. The exception to this is if
   * we are in said state, but the client itself disables fullscreen (3) then we
   * let the client go out of fullscreen while keeping fake fullscreen enabled
   * (as otherwise there will be a mismatch between the client and the window
   * manager's perception of the client's fullscreen state). */
  ...
  /* Some clients, e.g. firefox, will send a client message informing the window
   * manager that it is going into fullscreen after receiving the above signal.
   * This has the side effect of this function (setfullscreen) sometimes being
   * called twice when toggling fullscreen on and off via the window manager as
   * opposed to the application itself. To protect against obscure issues where
   * the client settings are stored or restored when they are not supposed to we
   * add an additional bit-lock on the old state so that settings can only be
   * stored and restored in that precise order. */
  if (savestate && !(c->oldstate & (1 << 1))) {
    ...
    if (restorefakefullscreen || c->fakefullscreen == 3)
      c->fakefullscreen = 1;
    /* The client may have been moved to another monitor whilst in fullscreen
     * which if tiled we address by doing a full arrange of tiled clients. If
     * the client is floating then the height and width may be larger than the
     * monitor's window area, so we cap that by ensuring max / min values. */
    if (c->isfloating) {
  ...
  /* Exception: if the client was in actual fullscreen and we exit out to fake
   * fullscreen mode, then the focus would sometimes drift to whichever window
   * is under the mouse cursor at the time. To avoid this we ask X for all
   * EnterNotify events and just ignore them.
   */
  if (!c->isfullscreen)
    while (XCheckMaskEvent(dpy, EnterWindowMask, &ev))
      ;
...
void togglefakefullscreen(const Arg *arg) {
  ...
  if (c->fakefullscreen != 1 &&
      c->isfullscreen) { // exit fullscreen --> fake fullscreen
    c->fakefullscreen = 2;
    setfullscreen(c, 0);
...
void togglefloating(const Arg *arg) {
  ...
  if (!c)
    return;
  if (c->isfullscreen &&
      c->fakefullscreen != 1) /* no support for fullscreen windows */
    return;
  c->isfloating = !c->isfloating || c->isfixed;
  if (selmon->sel->isfloating)
    // resize(c, c->x, c->y, c->w, c->h, 0);
    // Move floating window to center
    resizeclient(c, (selmon->mw - selmon->mw * 0.5) / 2,
                 (selmon->mh - selmon->mh * 0.5) / 2, selmon->mw * 0.5,
                 selmon->mh * 0.5);
  arrange(c->mon);
}
...
void xrdb(const Arg *arg) {
  ...
  for (i = 0; i < LENGTH(colors); i++)
    scheme[i] = drw_scm_create(drw, colors[i], 3);
  // Send notification on xrdb reload
  system("notify-send \"Colorscheme reload\" ");
  focus(NULL);
  arrange(NULL);
}
...
void distributetags(const Arg *arg) {
  unsigned int ui = 1;
  int i = 0;
